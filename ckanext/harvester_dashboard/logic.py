...
def get_harvester_job_dict(context, data_dict, harvest_source_ids_for_user):
    ...
    for harvest_job in harvest_jobs:
        ...
        if user_filter and job_not_set_filter:
            ...
            harvest_job_result = tk.get_action("harvest_job_show")(
                {"ignore_auth": True}, {"id": last_harvest_job_id}
            )
            last_harvest_job_dict[harvest_job["source_id"]] = HarvestJobInfo(
                last_job=harvest_job_result
            )
...
def get_harvest_source_name_dict(harvest_source_ids):
    ...
    harvest_source_name_dict = {
        package.id: PackageInfo(name=package.name) for package in packages
    }
...
def get_organizations_id_dict():
    ...
    organization_dict = {
        organization.id: OrganizationInfo(
...
def get_harvest_source_infos_for_user(context, data_dict):
    ...
    harvest_source_ids_for_user = get_harvest_source_ids_for_user(
        context, harvest_source_ids
    )

    harvest_sources_last_job_dict = get_harvester_job_dict(
        context, data_dict, harvest_source_ids_for_user
    )
